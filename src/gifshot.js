;(function(window, navigator, document, undefined) {var utils, videoStream, gifCodec, index;
utils = {
...
gifCodec = function () {
    ...
    function GifReader(buf) {
        var p = 0;
        // - Header.
        if (buf[p++] !== 71 || buf[p++] !== 73 || buf[p++] !== 70 || // GIF
            buf[p++] !== 56 || buf[p++] !== 57 || buf[p++] !== 97) {
            // 89a
        ...
        this.decodeAndBlitFrameBGRA = function (frame_num, pixels) {
            ...
            var op = (frame.y * width + frame.x) * 4;
            // output pointer.
    ...
    function GifReaderLZWOutputIndexStream(code_stream, p, output, output_length) {
        ...
        // Can be signed, we only use 20 bits.
        var prev_code = null;
        // Track code-1.
        while (true) {
            ...
            op += chase_length;
            var b = op;
            // Track pointer, writing backwards.
            if (chase_code !== code)
...
index = function (util) {
    videoStream.startVideoStreaming(function (cameraStream, videoElement, width, height) {
        var codecWriter, images = [], i = 0, canvas, img;
